...
""" print(conR.keys())
print(conR.ttl('logged:bicho@nunes')) """
""" testes() """
""" 
    login = 'logged:bicho@nunes'
    chaves = conR.keys()
    keys = []
    userLogin = conR.hget(f'user:' + 'bicho@nunes', 'loggin')
    status = userLogin.decode()
    for key in chaves:
        keys.append(key.decode())
    if login in keys:
        if status == 'off':
            print('Logando...')
            conR.hset('user:bicho@nunes', 'loggin', 'on')
        else:
            conR.expire('logged:bicho@nunes', 15)
            print('Você já está logado!')
    else:
        conR.hset('logged:bicho@nunes', 'timeExit', 0)
...
if 
    if status == 'off':
        conR.hset('logged:bicho@nunes', 'timeExit', 0)
        conR.expire('logged:bicho@nunes', 30)
        conR.hset('user:bicho@nunes', 'loggin', 'on')
    else:
        print('Você já está logado')
    ...
    time_to_expire_s = 10
    for i in range(10):
        conR.hset('loggin:' + 'bicho@nunes', 'logged',
                 json.dumps({
                     'value': i,
                     'expiration': time_to_expire_s
                 })) """